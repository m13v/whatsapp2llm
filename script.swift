...
func exportChats(startIndex: Int, count: Int) {
    ...
    for i in startIndex...(startIndex + count - 1) {
        ...
        if waitForExportCompletionAlertAndClickOK(timeout: 60) { // 60 second timeout
            ...
            // Add scrolling action here
            scrollChatList(pixels: 70) // Adjust the number of lines as needed
            usleep(1_000_000)  // Wait for 1 second after scrolling
        } else {
...
func scrollChatList(pixels: CGFloat) {
    guard let app = NSWorkspace.shared.runningApplications.first(where: { 
        $0.bundleIdentifier == "net.whatsapp.WhatsApp"
    }) else {
        print("WhatsApp is not running")
    ...
    if result != .success {
        print("Failed to get main window. Error: \(result)")
        return
    }
    guard let mainWindow = value as! AXUIElement? else {
        print("Could not cast main window to AXUIElement")
        return
    }

    // Get the position of the main window
    var positionValue: AnyObject?
    let positionResult = AXUIElementCopyAttributeValue(mainWindow, kAXPositionAttribute as CFString, &positionValue)
    if positionResult != .success {
        print("Failed to get window position. Error: \(positionResult)")
        return
    }
    ...
    if let axValue = positionValue as! AXValue? {
        var tempValue = CGPoint.zero
        AXValueGetValue(axValue, .cgPoint, &tempValue)
        position = tempValue
    } else if let pointValue = positionValue as? CGPoint {
        position = pointValue
    } else {
        print("Unexpected position value type: \(type(of: positionValue))")
        return
    }

    // Get the size of the main window
    var sizeValue: AnyObject?
    let sizeResult = AXUIElementCopyAttributeValue(mainWindow, kAXSizeAttribute as CFString, &sizeValue)
    if sizeResult != .success {
        print("Failed to get window size. Error: \(sizeResult)")
        return
    }
    
    print("Size value type: \(type(of: sizeValue))")
    
    var size: CGSize
    if let axValue = sizeValue as! AXValue? {
        var tempValue = CGSize.zero
        AXValueGetValue(axValue, .cgSize, &tempValue)
        size = tempValue
    } else if let sizeValue = sizeValue as? CGSize {
        size = sizeValue
    } else {
        print("Unexpected size value type: \(type(of: sizeValue))")
        return
    }

    print("Window position: \(position), size: \(size)")

    // Calculate a point in the left part of the window where the chat list likely is
    let scrollPoint = CGPoint(x: position.x + size.width * 0.75, y: position.y + size.height / 2)
    // Create a scroll wheel event
    // Perform multiple scroll attempts with delays
    for _ in 1...5 {  // Attempt to scroll 5 times
        // Move mouse to the scroll point
        let mouseMoveEvent = CGEvent(mouseEventSource: nil, mouseType: .mouseMoved, mouseCursorPosition: scrollPoint, mouseButton: .left)
        mouseMoveEvent?.post(tap: .cghidEventTap)
        
        // Perform scroll
        let scrollEvent = CGEvent(scrollWheelEvent2Source: nil, units: .pixel, wheelCount: 1, wheel1: Int32(-pixels / 5), wheel2: 0, wheel3: 0)
        scrollEvent?.post(tap: .cghidEventTap)
        
        print("Scroll event posted at point: \(scrollPoint)")
